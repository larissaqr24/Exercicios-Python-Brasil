'''Jogo de Craps. Faça um programa de implemente um jogo de Craps.
O jogador lança um par de dados, obtendo um valor entre 2 e 12. Se, na primeira jogada,
você tirar 7 ou 11, você um "natural" e ganhou. Se você tirar 2, 3 ou 12 na primeira jogada,
isto é chamado de "craps" e você perdeu. Se, na primeira jogada, você fez um 4, 5, 6, 8, 9 ou 10,
este é seu "Ponto". Seu objetivo agora é continuar jogando os dados até tirar este número novamente. 
Você perde, no entanto, se tirar um 7 antes de tirar este Ponto novamente.
'''

from itertools import count
import random

rodada = 1
ponto = 0 
while True:
    print ('Rodada {0}'. format(rodada))
    sorte = random.randrange(2,13)
    print ('Número sorteado: {0}'.format(sorte))
    if sorte == 7 or sorte == 11:
        print('Natural, Ganhou!')
        break
    elif sorte == 2 or sorte == 3 or sorte == 12:
        print('Craps, Perdeu!')
        break
    elif sorte == 4 or sorte == 5 or sorte == 6 or sorte == 8 or sorte == 9 or sorte == 10:
        print('Ponto.')
        ponto += 1
        rodada += 1
    elif rodada == 2:
        sorte == 4 or sorte == 5 or sorte == 6 or sorte == 8 or sorte == 9 or sorte == 10
        print(f'Número de pontos: {ponto}')
    else:
        sorte == 7
        print('Você perdeu!!!')